...
import styles from './Precrisis.module.css'

export function Precrisis() {
    ...
    async function fetchStudent() {
      ...
      if (docSnap.exists()) {
        const student = {
          ...
          precrisis: docSnap.data().precrisis
        }
        setStudent(student)
      } else {
    ...
    <>
      ...
      <main className={styles.container}>
        <div className={styles.content}>
          <h2 className={styles.title}>Formulários pré-crise de {student && student.name}</h2>

          <Link
            className={styles.addPrecrisisButton}
            to={`/precrisis/${student && student.id}/new`}
          >Cadastrar novo formulário Pré-Crise +</Link>

          <ul className={styles.studentList}>
            {student && student.precrisis && student.precrisis.map(record => (
              <li
